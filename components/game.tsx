...
import { ArrowDownUp, ArrowRight, Award, CalendarDays, Check, CheckCheck, ChevronRight, CircleHelp, Clock3, Copy, Flame, Infinity as InfinityIcon, Lightbulb, Loader2, LockKeyhole, Pin, Plus, Share2, Snowflake, Sparkles, Target, Trophy, Users, Volume2, VolumeX, X, Zap } from "lucide-react";
...
export default function Game() {
  ...
  function closeHelpAndFocusGame() {
    ...
    focusFrameRef.current = requestAnimationFrame(() => {
      focusFrameRef.current = null;
      if (request === requestRef.current && modalRef.current === null) inputRef.current?.focus({ preventScroll: true });
    });
  }
  ...
  async function submit(event?: FormEvent, isHint = false) {
    ...
    try {
      ...
      if (shouldReturnFocus) {
        focusFrameRef.current = requestAnimationFrame(() => {
          focusFrameRef.current = null;
          if (request !== requestRef.current || modalRef.current !== null) return;
  ...
  return <div className={`app-shell cosmetic-${selectedCosmetic}`}>
    <Toaster position="bottom-center" theme="dark" />
    <a className="skip-link" href="#game-board">Aller au jeu</a>
    <header className="site-header">
      ...
      <nav className="header-nav" aria-label="Navigation principale"><span className="nav-current">Le jeu</span><button onClick={() => setModal("stats")}>Ma progression</button><button onClick={() => setModal("help")}>Comment jouer</button></nav>
      <div className="header-actions"><span className="level-pill"><Sparkles size={15} />Niv. {level}</span><button className="icon-button" aria-label={profile.sound ? "Désactiver le son" : "Activer le son"} disabled={busy || loading} onClick={() => saveSound(!profile.sound)}>{profile.sound ? <Volume2 size={19} /> : <VolumeX size={19} />}</button><button className="icon-button mobile-help" aria-label="Comment jouer" onClick={() => setModal("help")}><CircleHelp size={20} /></button></div>
    </header>

    <main className="main-shell">
      <div className="page-heading"><div><div className="eyebrow"><span className="tiny-flame"><Flame size={13} /></span> FAITES CONFIANCE À VOTRE INTUITION</div><h1>Vous brûlez<span>.</span></h1><p>Un mot secret. Trouvez-le en vous rapprochant de son sens.</p></div><button className="streak-overview" onClick={() => setModal("stats")}><Flame size={24} /><span><strong>{streak} <small>jour{streak > 1 ? "s" : ""}</small></strong><span>de série quotidienne</span></span><ChevronRight size={16} /></button></div>
      ...
      <a className="text-action" href="/laboratoire">L’intrus · explorer le laboratoire →</a>
      <a className="text-action" href="/espace">Mon espace · parties conservées en ligne →</a>
      <a className="text-action" href="/cercles">Mes cercles →</a>
      <a className="text-action" href="/duels">Duels en trois manches →</a>
      <a className="text-action" href="/studio">Composer une création →</a>
      <ExpeditionPicker disabled={busy || loading} puzzle={puzzle} game={game} navigationGeneration={() => requestRef.current} onSelect={(seed, saved) => loadPuzzle("challenge", seed, saved)} />
      <CollectionPicker wins={profile.wins} seed={mode === "challenge" || mode === "free" ? puzzle?.seed : undefined} disabled={busy || loading} onSelect={seed => {
        ...
        window.history.replaceState(null, "", url); void loadPuzzle("challenge", seed);
      }} />
      ...
      <div className="play-layout">
        <div className="play-column">
          <section id="game-board" tabIndex={-1} className={`game-panel ${isAccepted ? "is-accepted" : ""} ${isRecord ? "is-record" : ""} ${solved ? "is-won" : ""} ${isCelebratingWin ? "is-celebrating" : ""}`} aria-label="Plateau de jeu">
...
{loadError ? <div className="load-error" role="alert"><CircleHelp size={34} /><h2>{mode === "archive" ? "Archive indisponible." : "Une petite pause imprévue."}</h2><p>{loadError}</p>{loadRetryable && <button className="primary-button" onClick={() => loadPuzzle(lastLoad.current.mode, lastLoad.current.locator, lastLoad.current.sessionSave)}>Réessayer<ArrowRight size={18} /></button>}{mode === "archive" && <button className="text-action" onClick={() => loadPuzzle("daily")}>Revenir au mot du jour<ArrowRight size={16} /></button>}</div> : <>
              <div className={`heat-scene ${isRecord ? "heat-record" : ""} ${isAccepted ? "heat-accepted" : ""}`} style={{ "--heat-color": best ? heat.color : "#ff754d", "--heat-intensity": String(.18 + Math.max(0, temperature) / 140) } as CSSProperties}>
                <div className="heat-atmosphere" aria-hidden="true" />
                <svg className="heat-dial" viewBox="0 0 330 264" aria-hidden="true"><defs><linearGradient id="dial-gradient" x1="0" y1="1" x2="1" y2="0"><stop stopColor="#ff754d" /><stop offset="1" stopColor="#ffcc9c" /></linearGradient></defs><circle className="dial-orbit" cx="165" cy="146" r="129" /><path className="dial-track" d="M 78.73 232.27 A 122 122 0 1 1 251.27 232.27" pathLength="100" /><path className="dial-value" d="M 78.73 232.27 A 122 122 0 1 1 251.27 232.27" pathLength="100" strokeDasharray={`${best ? Math.max(2, Math.min(100, temperature)) : 0} 100`} /><path className="dial-inner" d="M 98 213 A 95 95 0 1 1 232 213" /></svg>
                <div className="heat-center"><div className="thermo-caption">{solved ? "LA BRAISE EST ALLUMÉE" : "VOTRE MEILLEURE PISTE"}</div><div className={`temperature ${loading ? "is-loading" : ""}`}>{loading ? <Loader2 className="spinner" size={44} /> : <>{best ? temperature.toFixed(1).replace(".", ",") : "0"}<span>°</span></>}</div><div className="heat-status">{solved ? <Sparkles size={17} /> : best && temperature >= 35 ? <Flame size={17} /> : !best ? <Flame size={17} /> : <Snowflake size={17} />}{best ? heat.label : "À vous d’allumer la mèche"}</div></div>
                <div className="dial-labels"><span>FROID</span><span>BRÛLANT</span></div>
                {isRecord && <span className="record-toast"><Zap size={14} />Nouvelle meilleure piste !</span>}
                {isCelebratingWin && <div className="confetti" aria-hidden="true">{Array.from({ length: 20 }, (_, i) => <i key={i} style={{ "--i": i } as CSSProperties} />)}</div>}
              </div>
              {solved ? <div className="victory-box"><span className="victory-eyebrow">C’ÉTAIT BIEN</span><h2 ref={victoryRef} tabIndex={-1} aria-label={`Bravo, le mot était ${best?.word}`}>{best?.word}</h2><p>{game.guesses.length} essais, {game.hints ? `${game.hints} indice${game.hints > 1 ? "s" : ""}` : "aucun indice"}. Vous avez trouvé votre chemin.</p><div className="victory-actions"><button className="primary-button" onClick={shareResult}><Share2 size={17} />Partager ma victoire</button><button className="secondary-button" onClick={() => loadPuzzle("free", freshSeed())}>Encore un mot<ArrowRight size={17} /></button></div>{isCelebratingWin && <span className="xp-award"><Sparkles size={14} />Mot trouvé</span>}</div> : <div className="guess-area"><form onSubmit={submit} className="guess-form" aria-busy={busy}><label className="sr-only" htmlFor="guess">Votre proposition</label><input id="guess" ref={inputRef} value={input} onChange={e => { inputRevisionRef.current += 1; setInput(e.target.value); setError(""); }} placeholder="À quel mot pensez-vous ?" autoComplete="off" autoCorrect="off" autoCapitalize="none" spellCheck={false} enterKeyHint="send" maxLength={50} aria-describedby="guess-feedback" aria-invalid={Boolean(error)} disabled={loading} /><button type="submit" className="guess-button" aria-label={busy ? "Analyse en cours" : "Proposer ce mot"} disabled={loading || busy || !input.trim()}>{busy ? <Loader2 aria-hidden="true" className="spinner" size={21} /> : <ArrowRight size={24} />}</button></form><div className={`input-feedback ${error ? "error" : ""}`} id="guess-feedback" aria-live="polite">{error || (latest ? <><span style={{ color: warmth(latest.temperature).color }}>{latest.word} : {latest.temperature.toFixed(1).replace(".", ",")}°</span><span> {warmth(latest.temperature).label.toLowerCase()}.</span></> : <>Le sens compte. Pas les lettres. <span className="keyboard-hint">Entrée ↵</span></>)}</div></div>}
          ...
          <section className="history-panel"><div className="section-topline"><h2>Vos explorations<span>{game.guesses.length}</span></h2><button className="sort-button" onClick={() => setSort(sort === "heat" ? "recent" : "heat")}><ArrowDownUp size={14} />{sort === "heat" ? "Les plus chauds" : "Les plus récents"}</button></div>
        ...
        <aside className="side-column">
          <section className="progress-card"><div className="section-topline"><span className="card-eyebrow">VOTRE ÉTINCELLE</span><span className="small-icon"><Zap size={17} /></span></div><div className="player-level"><span className="level-emblem"><Flame size={27} /></span><div><h2>{level < 3 ? "L’étincelle" : level < 7 ? "La flamme" : "Le brasier"}</h2><span>Niveau {level} · {xp.toLocaleString("fr-FR")} XP</span></div></div><Progress className="xp-track" value={(xp % 500) / 5} aria-label="Progression vers le prochain niveau" aria-valuetext={`${xp % 500} sur 500 XP`} /><div className="xp-caption"><span>{500 - xp % 500} XP avant le niveau {level + 1}</span><Sparkles size={12} /></div><div className="personal-stats"><div><strong>{profile.wins.length}</strong><span>Mots trouvés</span></div><div><strong>{streak}<Flame size={15} /></strong><span>Jours de série</span></div></div><button className="text-action" onClick={() => setModal("stats")}>Ma progression<ArrowRight size={16} /></button></section>

          {ritual && <WeeklyRitualCard projection={ritual} />}

          <section className="notebook-card"><div className="section-topline"><h2><Pin size={17} />Mon carnet</h2><span className="muted-count">{game.pins.length}/3</span></div><p>Gardez vos pistes pour rebondir.</p><div className="notebook-pins">{game.pins.map(word => <button key={word} disabled={busy || loading} onClick={() => pin(word)} title="Détacher cette piste">{word}<X size={13} /></button>)}{Array.from({ length: 3 - game.pins.length }, (_, i) => <span className="empty-pin" key={i}><Plus size={14} /></span>)}</div>{!game.pins.length && <span className="notebook-note">Épinglez un mot dans vos explorations.</span>}</section>

          <section className="tip-card"><span className="tip-icon"><Lightbulb size={20} /></span><div><span className="card-eyebrow">LE DÉCLIC</span><h3>Pensez autour du mot.</h3><p>« Plage » peut être proche de « vacances ». Les mots partagent un univers, pas forcément une définition.</p></div></section>

          <button className="friend-card" onClick={() => setModal("challenge")}><div className="friend-card-top"><span className="friends-glyph"><Users size={23} /></span><ArrowRight size={20} /></div><h2>Qui aura le déclic ?</h2><p>Un même mot. Vos amis.<br />À chacun son chemin.</p><span className="friend-link">Lancer un défi<ChevronRight size={15} /></span></button>
      ...
      <AdPlacement />
      <footer className="site-footer"><a className="footer-brand" href="/">braise.</a><span>Un nouveau mot. Un nouveau déclic.</span><div><button onClick={() => setModal("about")}>À propos</button><button onClick={() => setModal("privacy")}>Confidentialité</button><button onClick={() => setModal("help")}>Les règles</button></div><span className="footer-language">FR <span>·</span> Fait pour les curieux</span></footer>
    </main>

    <Dialog open={modal !== null} onOpenChange={open => !open && setModal(null)}><DialogContent className="braise-dialog"><DialogHeader><span className="modal-symbol">{modal === "stats" ? <Trophy /> : modal === "challenge" ? <Users /> : modal === "privacy" ? <LockKeyhole /> : <Flame />}</span><DialogTitle>{modal === "help" ? "Un mot. Et le déclic." : modal === "stats" ? "Votre feu prend." : modal === "challenge" ? "Passez le mot. Pas la réponse." : modal === "privacy" ? "Votre partie vous appartient." : modal === "share" ? "À vous de partager." : "Bienvenue chez Braise."}</DialogTitle><DialogDescription>{modal === "help" ? "La règle tient en trois petites étapes." : modal === "stats" ? "Votre progression personnelle, enregistrée dans ce navigateur." : modal === "challenge" ? "Créez un lien : chaque personne cherchera exactement le même mot." : modal === "privacy" ? "Le fonctionnement de cette première version." : modal === "share" ? "Copiez ce texte pour le partager avec vos amis." : "Un jeu de proximité sémantique, en français."}</DialogDescription></DialogHeader>
